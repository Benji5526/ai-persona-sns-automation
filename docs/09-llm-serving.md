...
flowchart LR
    subgraph VLLM["vLLM 서버 (상시 구동)"]
        BASE["베이스 모델<br/>(예: Qwen/Llama 계열, 페르소나 게시 언어에 맞춰 선정)"]
        L1["LoRA: persona_001"]
        L2["LoRA: persona_002"]
        L3["LoRA: persona_00N"]
        BASE --- L1
...
- 페르소나가 실제로 게시할 언어에서의 응답 품질 (타겟 국가를 정하지 않는 글로벌 자연 유입 모델이므로,
  언어는 "이용자 국적"이 아니라 "페르소나가 어떤 언어로 말하기로 했는가"로 결정됨 — 다국어 대응이
  필요하면 언어별 검증 항목으로 확장)
...
| 방식 | 장점 | 단점 | 적합한 단계 |
|---|---|---|---|
| 프롬프트 기반 (캐릭터 시트 + few-shot) | 데이터 수집·학습 파이프라인 불필요, 즉시 반복 실험 가능 | 요청마다 컨텍스트 토큰 소모, 긴 대화에서 톤이 흔들릴 수 있음 | [로드맵](08-roadmap.md) MVP~Phase 1 (수동 파일럿·반자동 도입) |
