...
[07-compliance.md](07-compliance.md) 2b에서 확정한 대로, EU AI Act Article 50(1)에 따라 **신규 대화 스레드마다
최초 1회, 캐릭터 톤을 유지한 채 AI/가상 캐릭터임을 고지**합니다. 이 고지는 2번 섹션의 휴먼라이크
페이싱과 상충하지 않습니다 — "매 응답을 로봇처럼 만들라"가 아니라 "대화 시작 시점에 한 번 명확히
알리라"는 의무이기 때문입니다.

- **시점**: 스레드의 첫 응답 생성 시 (사람 상대의 첫 메시지에 대한 답장 직전 또는 그 안에 포함)
- **방식 예시**: "안녕하세요! 저는 OO이에요 :) 참고로 저는 AI 페르소나라서, 답장이 조금 걸릴 수 있어요 —
  그래도 진짜 대화처럼 신경 써서 답할게요!" 같이 캐릭터 시트의 말투를 그대로 유지
- **추적**: [06-data-model.md](06-data-model.md)의 `conversation.ai_disclosure.disclosed_at` 필드로 스레드별
  고지 여부/시점을 기록 — null이면 응답 생성 전 반드시 고지 문구를 선삽입
- **계정 레벨 고지**: 스레드 내 1회 고지와 별개로, 계정 소개/고정 게시물에도 가상 캐릭터임을 상시 고지
  ([07-compliance.md](07-compliance.md) 2b 참고)
...
- [ ] 대화 로그 보관 정책 및 개인정보 처리 방침 ([07-compliance.md](07-compliance.md) 4번, 조지아 개인정보보호법 기준)
- [ ] 신규 스레드 최초 1회 AI 고지 메시지 문구를 페르소나별로 확정 (4번 참고, 캐릭터 톤 유지)
