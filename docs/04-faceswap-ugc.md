# 04. 얼굴교체 엔진 — 구매 UGC 페이스스왑 (이미지/영상)
...
> **이 문서는 "얼굴교체 엔진" 하나를 다룹니다.** 입력이 이미지든 영상이든 **동일한 얼굴 교체 모델**
> (예: inswapper/GHOST/SimSwap류)을 공유하며, 차이는 단일 프레임 처리냐 프레임별 처리 + 시계열 보정이냐뿐입니다
> — 그래서 두 경우를 별도 파이프라인으로 나누지 않고 하나의 엔진으로 묶습니다.
>
> 반대로 "이미지 속 인물의 정체성으로 별도 영상의 움직임을 재현"하는 **모션 리타겟팅/애니메이션**
> (예: LivePortrait/Hallo류)은 픽셀을 덮어씌우는 게 아니라 모델이 프레임을 재생성하는, 기술적으로
> 완전히 다른 엔진입니다 — [14-motion-reenactment.md](14-motion-reenactment.md)에서 별도로 다룹니다.
...
동일한 얼굴 교체 모델을 입력 타입에 따라 다르게 호출합니다 (엔진은 하나, 처리 단위만 분기).
